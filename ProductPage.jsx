    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex gap-4">
                <div className="w-16 space-y-4">
                  {product.images.map((src, index) => (
                    ...
                    >
                      <img src={src} alt={`Product thumbnail ${index + 1}`} width={60} height={60} className="cursor-pointer" />
                    </div>
                  ))}
                </div>
          ...
          {/* Middle - Product Info */}
          <div className="lg:w-1/3">
            <p className="text-sm">Brand: <span className="text-[#007185]">{product.brand}</span></p>
            <h1 className="text-xl font-medium mt-2">{product.name}</h1>
            
            <div className="flex items-center mt-2">
              <div className="flex">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} className={`h-4 w-4 ${star <= product.reviews_avg ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} />
                ))}
              </div>
              <a href="#reviews" className="text-[#007185] ml-2 text-sm">{product.reviews_count} ratings</a>
            </div>
          ...
          <div className="lg:w-1/3">
            <div className="border rounded-lg p-4">
              <div className="flex items-baseline">
                ...
                <span className="text-3xl font-medium mx-1">{product.price}</span>
              </div>
              ...
              <div className="mt-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    <span className="font-medium">SAR96 delivery</span> {product.delivery_time}.
                  </span>
                </div>
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="md:w-64">
              <div className="flex items-center mb-2">
                <span className="font-medium mr-2">{product.reviews_avg} out of 5</span>
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star key={star} className={`h-4 w-4 ${star <= product.reviews_avg ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} />
                  ))}
              ...
              <p className="text-sm text-[#007185]">{product.reviews_count} global ratings</p>
                  ...
                  <div key={rating} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 h-4 bg-gray-200 rounded">
                      <div
                        ...
                        style={{ width: `${(product.reviews.filter(r => r.rating === rating).length / product.reviews.length) * 100}%` }}
                      />
                    </div>
                    <span className="text-sm w-8">
                      {Math.round((product.reviews.filter(r => r.rating === rating).length / product.reviews.length) * 100)}%
                    </span>
                  </div>
                ))}
              </div>
